#!/usr/bin/env python3
"""
3D Point Cloud Inference with Global Alignment Script

This script performs inference using the ARCroco3DStereo model with global alignment
optimization and visualizes the resulting 3D point clouds with the PointCloudViewer.
...
def collate_with_cat(whatever, lists=False):
    """Collate and concatenate tensors from nested structures."""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run 3D point cloud inference with global alignment using ARCroco3DStereo."
    )
    parser.add_argument(
        "--model_path",
        type=str,
...
def prepare_input(
    ...
    Uses forward_backward_permutations to generate multiple view pairs for global alignment.
    ...
    Returns:
        list: A list of view groups (each group is a list of views for pairwise inference).
    """
...
def prepare_output(output, outdir, device, ga_niter=300, ga_lr=0.01, ga_schedule="linear"):
    """
    Process inference outputs with global alignment optimization.
    
    Args:
        output: Dictionary containing view1, view2, pred1, pred2
        outdir: Output directory
        device: Device to run optimization on
        ga_niter: Number of iterations for global alignment
        ga_lr: Learning rate for global alignment
        ga_schedule: Learning rate schedule ("linear" or "cosine")
    ...
    try:
        ...
        raise ImportError(
            "cloud_opt module not found. Please copy the cloud_opt directory from CUT3R to TTT3R, "
...
def parse_seq_path(p, frame_interval=1):
    """Parse sequence path (directory or video file)."""
    if os.path.isdir(p):
...
def run_inference(args):
    """
    Execute the full inference and visualization pipeline with global alignment.
    ...
    # Prepare input views with permutations for pairwise inference.
    print("Preparing input views with permutations...")
    views = prepare_input(
        img_paths=img_paths,
        img_mask=img_mask,
        size=args.size,
    ...
    # Run inference on all view pairs.
    print("Running inference on view pairs...")
    ...
    print(
        f"Inference completed in {total_time:.2f} seconds (average {per_frame_time:.2f} s per view group)."
    )

    # Process outputs with global alignment.
    print("Running global alignment optimization...")
    pts3ds_other, colors, conf, cam_dict = prepare_output(
        output, args.output_dir, device, args.ga_niter, args.ga_lr, args.ga_schedule
    )
